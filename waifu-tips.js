...
live2d_settings['waifuMinWidth']        = 'disable';    // 面页小于 指定宽度 隐藏看板娘，例如 'disable'(禁用), '768px' 这段代码是wap端隐藏的关键
...
live2d_settings['aboutPageUrl']         = 'http://www.live2dkanbanliang.com/';   // 关于页地址, '{URL 网址}'
...
function loadTipsMessage(result) {
    ...
    $('.waifu-tool .fui-cross').click(function(){
        ...
        //看板娘关闭后
        $("#blog_setting_live2D_switch").prop("checked",false);
    });
    ...
    function loadOtherModel() {
        ...
        //cache属性是true（默认值）时：在第一次请求完成之后，如果地址和参数不变化，
        // 第二次去请求，会默认获取缓存中的数据，不去读取服务器端的最新数据。
        $.ajax({
            cache: modelRandMode == 'switch' ? true : false,
            url: live2d_settings.modelAPI+modelRandMode+'/?id='+modelId,
            dataType: "json",
            success: function(result) {
                console.log("看板娘模板为"+result.model['id']);
    ...
    function loadRandTextures() {
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function(result) {
                console.log("服装id为"+result.textures['id']);
